...
@implementation SandCardNoAuthToolView
...
+ (instancetype)createAuthToolViewOY:(CGFloat)OY{
    ...
    SandCardNoAuthToolView *view = [[SandCardNoAuthToolView alloc] initWithFrame:CGRectMake(0, OY, 0, 0)];
    return view;
...
- (instancetype)initWithFrame:(CGRect)frame{
    ...
    if ([super initWithFrame:frame]) {
        self.titleLab.text = @"杉德卡卡号";
        self.textfiled.placeholder = @"输入杉德卡卡号";
...
- (void)textFieldDidEndEditing:(UITextField *)textField{
    ...
    if (((![self validateBankCard:textField.text]) || !(textField.text.length == 16)) && (textField.text.length>0)) {
