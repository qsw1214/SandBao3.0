...
-(void)setup
{
    ...
    //消息体
    if (self.alertViewStyle == CustomAlertViewStyleDefault){
        //消息体
        if (_messageFont) {
            msgFont = [UIFont systemFontOfSize:16.0f];
        }else {
            msgFont = _messageFont;
        }
    ...
    //authCode
    if (self.alertViewStyle == CustomAlertViewStyleAutnCodeTextInput){
        commImageView = [[UIImageView alloc] init];
        commImageView.image = _commImage;
        commImageView.translatesAutoresizingMaskIntoConstraints = NO;
        commImageView.contentMode = UIViewContentModeScaleAspectFit;
        [contentView addSubview:commImageView];
        
        if (_commTextFiledFont) {
            authCodeFont = [UIFont systemFontOfSize:16.0f];
        }else {
            authCodeFont = _commTextFiledFont;
        }
        
        _commTextFiled = [[UITextField alloc] init];
        _commTextFiled.translatesAutoresizingMaskIntoConstraints = NO;
        _commTextFiled.font = authCodeFont;
        _commTextFiled.clearButtonMode = UITextFieldViewModeWhileEditing;
        _commTextFiled.placeholder = _commTextFiledplaceholder;
        _commTextFiled.keyboardType = UIKeyboardTypeNumberPad;
        _commTextFiled.textColor = [UIColor blackColor];
        [contentView addSubview:_commTextFiled];
    }
    ...
    if (self.alertViewStyle == CustomAlertViewStyleDefault) {
        bindDic = @{@"titleLabel":titleLabel,@"msgLabel":_msgLabel,@"lineView":lineView,@"relateBtn":relateBtn};
        paramDic = @{@"topMargin":@(kMarginTopButtom),@"lineHeight":@(1.0),@"msgMarginLeftRight":@(17.0f),@"marginTop":@(kMarginTopButtom),@"marginLeft":@(kMarginLeftRight)};
    } else if (self.alertViewStyle == CustomAlertViewStyleAutnCodeTextInput) {
        bindDic = @{@"titleLabel":titleLabel,@"commImageView":commImageView, @"commTextFiled":_commTextFiled, @"lineView":lineView,@"relateBtn":relateBtn};
        paramDic = @{@"topMargin":@(kMarginTopButtom),@"negativeTopMargin":@(kMarginToptop),@"kSpacing":@(kSpacing),@"lineHeight":@(1.0),@"msgMarginLeftRight":@(17.0f),@"marginTop":@(kMarginTopButtom),@"marginLeft":@(kMarginLeftRight)};
    }
    ...
    if (self.alertViewStyle == CustomAlertViewStyleDefault) {
        ...
        //authCode约束
        NSArray *authCodeImageView_H = [NSLayoutConstraint constraintsWithVisualFormat:@"H:|-msgMarginLeftRight-[commImageView]-kSpacing-[commTextFiled(>=100)]-msgMarginLeftRight-|" options:0 metrics:paramDic views:bindDic];
        [contentView addConstraints:authCodeImageView_H];
        
        NSString *formatStr = @"V:|-topMargin-[titleLabel]-topMargin-[commImageView]-negativeTopMargin-[commTextFiled]-topMargin-[lineView(lineHeight)][relateBtn]|";
        NSArray *views_V = [NSLayoutConstraint  constraintsWithVisualFormat:formatStr options:0 metrics:paramDic views:bindDic];
        [contentView addConstraints:views_V];
    }
