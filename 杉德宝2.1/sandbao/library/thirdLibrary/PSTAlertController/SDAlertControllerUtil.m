...
+ (void)showAlertControllerDefult:(NSString*)title actionBlock:(SDAlertControllerActionBlock)actionBlock{
    ...
    PSTAlertController *pstaC = [PSTAlertController alertControllerWithTitle:@"温馨提示" message:title preferredStyle:PSTAlertControllerStyleAlert];
    [pstaC addAction:[PSTAlertAction actionWithTitle:@"" handler:^(PSTAlertAction * _Nonnull action) {
        actionBlock(0);
    }]];
    
    [pstaC showWithSender:nil controller:[UIApplication sharedApplication].keyWindow animated:YES completion:nil];
