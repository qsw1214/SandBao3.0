//
//  BindingBankSecondViewController.m
//  sandbao
//
...
@interface BindingBankSecondViewController ()<UITextFieldDelegate>
{
...
@implementation BindingBankSecondViewController
...
-(void)addView:(UIView *)view
{
    ...
    bankLabel.text = @"银行卡";
    bankLabel.textColor = textFiledColor;
    ...
    nameLabel.text = @"姓名";
    nameLabel.textColor = textFiledColor;
    ...
    IDNumLabel.text = @"证件号";
    IDNumLabel.textColor = textFiledColor;
    ...
    IDNumTextField.placeholder = @"身份证";
    IDNumTextField.text = _identityNo;
    IDNumTextField.userInteractionEnabled = NO;
    ...
    CGFloat bankViewHeight = bankLabelSize.height + 2 * sapce;
    ...
    [authToolsView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(bankAreaView.mas_bottom).offset(authToolsViewTop);
        make.left.equalTo(scrollView).offset(0);
    ...
    scrollView.contentSize = CGSizeMake(viewSize.width, bankAreaViewHeight + authToolsViewH + authToolsViewTop + agreementAreaViewHeight + authenticationBtnTop + authenticationBtnHeight + 100);
...
- (void)verificationInput
{
    ...
    if (IDNumTextField.text.length > [IDNumTextField.maxLenght intValue] ) {
