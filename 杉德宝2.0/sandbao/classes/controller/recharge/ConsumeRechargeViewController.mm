//
//  ConsumeRechargeViewController.m
//  sandbao
//
...
@interface ConsumeRechargeViewController ()<UITextFieldDelegate>
{
...
@implementation ConsumeRechargeViewController
...
- (void)verificationInput
{
    ...
    [payView setPayInfo:(NSArray*)payModeArray moneyStr:[NSString stringWithFormat:@"¥%@",moneyTextField.text] orderTypeStr:@"消费账户充值"];
    [self fee];
...
- (void)payViewForgetPwd:(NSString *)type{
    ...
    if ([type isEqualToString:PAYTOOL_ACCPASS]) {
        ...
        NSMutableDictionary *accountDic = [consumePayToolDic objectForKey:@"account"];
...
- (void)showDescribrInfoFromPayToolDic:(NSDictionary*)dic{
    ...
    if ([@"1003" isEqualToString:[dic objectForKey:@"type"]]) {
         _userableBalanceFloat = [[[dic objectForKey:@"account"] objectForKey:@"useableBalance"] doubleValue]/100;
...
- (void)load
{
    ...
    [[SDRequestHelp shareSDRequest] dispatchGlobalQuque:^{
        ...
        NSString *payTool = [[PayNucHelper sharedInstance] dictionaryToJson:consumePayToolDic];
...
- (void)fee
{
    ...
    [[SDRequestHelp shareSDRequest] dispatchGlobalQuque:^{
        ...
        NSString *inPayTool = [[PayNucHelper sharedInstance] dictionaryToJson:consumePayToolDic];
