//
//  SandCardRechargeViewController.m
//  sandbao
//
...
@interface SandCardRechargeViewController ()<UITextFieldDelegate>
{
...
@property (nonatomic, assign) float userableBalanceFloat;
...
@implementation SandCardRechargeViewController
...
{
    //导航渐变条
    navCoverView = [NavCoverView shareNavCoverView:CGRectMake(0, 0, viewSize.width, controllerTop) title:@"杉德卡主账户充值"];
...
-(void)addView:(UIView *)view
{
    ...
    payModeBtn.userInteractionEnabled = YES;
    [scrollView addSubview:payModeBtn];
    
    UIImage *iconImage = [UIImage imageNamed:@"list_sand_logo"];
    ...
    bankView.userInteractionEnabled = NO;
    bankView.backgroundColor = [UIColor clearColor];
    [payModeBtn addSubview:bankView];
    ...
    [bankView addSubview:bankNametLabel];
    
    balanceLab = [[UILabel alloc] init];
    balanceLab.text = @"主账户余额:0.00元";
    balanceLab.textColor = RGBA(255, 102, 57, 1.0);
    balanceLab.font = [UIFont systemFontOfSize:balaceLabSize];
    [bankView addSubview:balanceLab];
    ...
    // 最高限额充值按钮
    UIButton *limitbtn = [[UIButton alloc] init];
    [limitbtn setTag:3];
    [limitbtn setTitle:@"最高限额充值" forState: UIControlStateNormal];
    [limitbtn setTitleColor:RGBA(65, 131, 215, 1.0) forState:UIControlStateNormal];
    limitbtn.titleLabel.font = [UIFont systemFontOfSize:labelTextSize];
    [limitbtn addTarget:self action:@selector(buttonActionToDoSomething:) forControlEvents:UIControlEventTouchUpInside];
    [moneyView addSubview:limitbtn];
    ...
    CGSize balanceLabelSize = [balanceLab sizeThatFits:CGSizeMake(bankNametLabelWidth, MAXFLOAT)];
    CGFloat bankViewHeight = bankNametLabelSize.height + space + balanceLabelSize.height;
    
    CGFloat payModeBtnHeight = bankViewHeight + 2 * leftRightSpace;
    ...
    CGSize limitbtnSize = [limitbtn.titleLabel.text sizeWithNSStringFont:limitbtn.titleLabel.font];
    CGFloat limitbtnHeight = limitbtnSize.height + 2 * 5;
    CGFloat moneyTextFieldWidth = commWidth - moneyTitleLabelSize.width - space -limitbtnSize.width - space;
    ...
    [bankView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.centerY.equalTo(payModeBtn);
        make.size.mas_equalTo(CGSizeMake(bankNametLabelWidth, bankViewHeight));
    }];
    
    [bankNametLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(bankView).offset(0);
    ...
    [balanceLab mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(bankNametLabel.mas_bottom).offset(space);
        make.left.equalTo(bankView).offset(0);
        make.size.mas_equalTo(CGSizeMake(bankNametLabelWidth, balanceLabelSize.height));
    }];
    
    [arrowImageView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(bankView.mas_right).offset(space);
        make.centerY.equalTo(payModeBtn);
    }];
    ...
    [limitLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.size.mas_equalTo(CGSizeMake(commWidth, limitbtnSize.height));
    }];
    ...
    [moneyTextField mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.centerY.equalTo(moneyView);
        make.size.mas_equalTo(CGSizeMake(moneyTextFieldWidth, moneyViewHeight));
    }];
    
    [limitbtn mas_makeConstraints:^(MASConstraintMaker *make) {
        make.right.equalTo(moneyView.mas_right).offset(-leftRightSpace);
        make.centerY.equalTo(moneyView);
        make.size.mas_equalTo(CGSizeMake(limitbtnSize.width, limitbtnHeight));
    }];
...
- (void)verificationInput
{
    ...
    if (_limitFloat < [moneyTextField.text floatValue]) {
        [Tool showDialog:@"输入金额超过可用额度"];
        [self changeBtnBackGround:nil];
        moneyTextField.text = @"";
        return;
    }
    if (_userableBalanceFloat < [moneyTextField.text floatValue]) {
        [Tool showDialog:@"输入金额超过该主账户可用余额"];
        [self changeBtnBackGround:nil];
        moneyTextField.text = @"";
        return;
    }
    ...
    [payView setPayInfo:(NSArray*)payModeArray moneyStr:[NSString stringWithFormat:@"¥%@",moneyTextField.text] orderTypeStr:@"记名卡转入"];
...
- (void)buttonActionToDoSomething:(id)sender
{
    ...
    switch (btn.tag) {
        ...
        case 3:
        {
            moneyTextField.text = [NSString stringWithFormat:@"%.2f",_limitFloat>_userableBalanceFloat?_userableBalanceFloat:_limitFloat];
...
- (void)payViewSelectPayToolDic:(NSMutableDictionary *)selectPayToolDict{
    ...
    [self showDescribrInfoFromPayToolDic:outPayToolDic];
    _limitFloat = [[PayNucHelper sharedInstance] limitInfo:[outPayToolDic objectForKey:@"limit"]]/100;
    limitLabel.text = [NSString stringWithFormat:@"该账户本次最多可充值:%.2f元",_limitFloat];
}
...
- (void)payViewForgetPwd:(NSString *)type{
    ...
    if ([type isEqualToString:PAYTOOL_ACCPASS]) {
        ...
        NSMutableDictionary *accountDic = [sandCardPayToolDic objectForKey:@"account"];
...
/**
 ...
 @param dic 支付工具dic
 */
- (void)showDescribrInfoFromPayToolDic:(NSDictionary*)dic{
    
    if ([@"1001" isEqualToString:[dic objectForKey:@"type"]]) {
        _userableBalanceFloat = 99999999;
        balanceLab.text = @"快捷借记卡";
    }else
    if ([@"1002" isEqualToString:[dic objectForKey:@"type"]]) {
        _userableBalanceFloat = 99999999;
        balanceLab.text = @"快捷贷记卡";
    }else
    if ([@"1003" isEqualToString:[dic objectForKey:@"type"]]) {
        _userableBalanceFloat = [[[dic objectForKey:@"account"] objectForKey:@"useableBalance"] floatValue]/100;
        balanceLab.text = [NSString stringWithFormat:@"主账户可用余额%.2f元",_userableBalanceFloat];;
    }else
    if ([@"1004" isEqualToString:[dic objectForKey:@"type"]]) {
        _userableBalanceFloat = [[[dic objectForKey:@"account"] objectForKey:@"useableBalance"] floatValue]/100;
        balanceLab.text = [NSString stringWithFormat:@"主账户可用余额%.2f元",_userableBalanceFloat];;
    }else
    if ([@"1005" isEqualToString:[dic objectForKey:@"type"]]) {
        _userableBalanceFloat = [[[dic objectForKey:@"account"] objectForKey:@"useableBalance"] floatValue]/100;
        balanceLab.text = [NSString stringWithFormat:@"主账户可用余额%.2f元",_userableBalanceFloat];;
    }else
    if ([@"1006" isEqualToString:[dic objectForKey:@"type"]]) {
        _userableBalanceFloat = [[[dic objectForKey:@"account"] objectForKey:@"useableBalance"] floatValue]/100;
        balanceLab.text = [NSString stringWithFormat:@"主账户可用余额%.2f元",_userableBalanceFloat];;
    }else
    if ([@"1007" isEqualToString:[dic objectForKey:@"type"]]) {
        _userableBalanceFloat = 99999999;
        balanceLab.text = @"久璋宝杉德币账户";
    }else
    if ([@"1008" isEqualToString:[dic objectForKey:@"type"]]) {
        _userableBalanceFloat = 99999999;
        balanceLab.text = @"久璋宝专用账户";
    }else
    if ([@"1009" isEqualToString:[dic objectForKey:@"type"]]) {
        _userableBalanceFloat = 99999999;
        balanceLab.text = @"久璋宝通用账户";
    }else
    if ([@"1010" isEqualToString:[dic objectForKey:@"type"]]) {
        _userableBalanceFloat = 99999999;
        balanceLab.text = @"会员卡账户";
    }else
    if ([@"1011" isEqualToString:[dic objectForKey:@"type"]]) {
        _userableBalanceFloat = 99999999;
        balanceLab.text = @"网银借记卡";
    }else
    if ([@"1012" isEqualToString:[dic objectForKey:@"type"]]) {
        _userableBalanceFloat = 99999999;
        balanceLab.text = @"网银贷记卡";
    }else{
        _userableBalanceFloat = 99999999;
        balanceLab.text = @"   ";
    }
...
- (void)load
{
    ...
    [[SDRequestHelp shareSDRequest] dispatchGlobalQuque:^{
        ...
        paynuc.set("tTokenType", [_tTokenType UTF8String]);
        ...
        NSString *payTool = [[PayNucHelper sharedInstance] dictionaryToJson:sandCardPayToolDic];
        ...
        [[SDRequestHelp shareSDRequest] requestWihtFuncName:@"payTool/getPayTools/v1" errorBlock:^(SDRequestErrorType type) {
            ...
            [[SDRequestHelp shareSDRequest] dispatchToMainQueue:^{
                ...
                if ([outpayToolsArray count] <= 0) {
                    [Tool showDialog:@"无可用充值方式,请绑定新卡" defulBlock:^{
                        BindingSandViewController *mBindingSandViewController = [[BindingSandViewController alloc] init];
                        [self.navigationController pushViewController:mBindingSandViewController animated:YES];
                    }];
                } else {
                    ...
                    //2.设置VC默认显示的支付
                    outPayToolDic = [[NSMutableDictionary alloc] initWithDictionary:payToolsArrayUsableM[0]];
                    [self resetBankNameLabelAndIconImageView];
...
- (void)resetBankNameLabelAndIconImageView{
    ...
    _limitFloat = [[PayNucHelper sharedInstance] limitInfo:[payToolsArrayUsableM[0] objectForKey:@"limit"]]/100;
    [self showDescribrInfoFromPayToolDic:payToolsArrayUsableM[0]];
    limitLabel.text = [NSString stringWithFormat:@"该账户本次最多可充值:%.2f元",_limitFloat];
}
/**
 *@brief 初始化支付方式
...
- (void)fee
{
    ...
    [[SDRequestHelp shareSDRequest] dispatchGlobalQuque:^{
        ...
        NSString *inPayTool = [[PayNucHelper sharedInstance] dictionaryToJson:sandCardPayToolDic];
