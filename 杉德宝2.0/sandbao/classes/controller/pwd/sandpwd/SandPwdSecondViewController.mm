...
@interface SandPwdSecondViewController ()<UITextFieldDelegate>
{
...
@implementation SandPwdSecondViewController
...
- (CGFloat)addAuthToolsView:(UIView *)view
{
    ...
    for (int i = 0; i < regAuthToolsArrayCount; i++) {
        ...
        if (i == 0) {
            ...
            if (i > 0 && i < regAuthToolsArrayCount) {
...
- (void)verificationInput
{
    ...
    //支付密码过于简单校验
    BOOL payPassRegular = [UITextField payPassWdTooSimple:accPwdTextField.text];
    if (payPassRegular == NO) {
        [Tool showDialog:@"密码过于简单" defulBlock:^{
            accPwdTextField.text = @"";
        }];
        return;
    }
    [self submit];
...
- (void)submit
{
    ...
    [[SDRequestHelp shareSDRequest] dispatchGlobalQuque:^{
        ...
        [regAuthToolsArray1 addObject:authToolsDic];
        ...
        [[SDRequestHelp shareSDRequest] requestWihtFuncName:@"authTool/setRegAuthTools/v1" errorBlock:^(SDRequestErrorType type) {
            ...
            [[SDRequestHelp shareSDRequest] dispatchToMainQueue:^{
                ...
                [Tool showDialog:@"卡密修改成功" defulBlock:^{
