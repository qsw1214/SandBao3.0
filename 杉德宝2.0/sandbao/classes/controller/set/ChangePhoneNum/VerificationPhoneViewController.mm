...
@interface VerificationPhoneViewController ()<UITextFieldDelegate,SDPayViewDelegate>
{
...
- (void)settingNavigationBar
{
    ...
    //导航渐变条
    navCoverView = [NavCoverView shareNavCoverView:CGRectMake(0, 0, viewSize.width, controllerTop) title:@"身份验证"];
...
- (void)payViewPwd:(NSString *)pwdStr paySuccessView:(SDPaySuccessAnimationView *)successView{
    ...
    [self vericationSuccessBlock:^(NSArray *paramArr){
        ...
        dispatch_after(delayTime, dispatch_get_main_queue(), ^{
            [self resetPhoneNo:_phoneNoStrNew authTools:paramArr[0]];// 重置手机号码
        });
 ...
 *@return NSArray
 */
- (void)resetPhoneNo:(NSString*)phoneNumber authTools:(NSString*)authTools
{
    HUD = [SDMBProgressView showSDMBProgressOnlyLoadingINViewImg:self.view];
    [SDRequestHelp shareSDRequest].HUD = HUD;
    [SDRequestHelp shareSDRequest].controller = self;
    [[SDRequestHelp shareSDRequest] dispatchGlobalQuque:^{
        ...
        [userInfoDic setValue:phoneNumber forKey:@"phoneNo"];
        NSString *userInfo = [[PayNucHelper sharedInstance] dictionaryToJson:userInfoDic];
        paynuc.set("userInfo", [userInfo UTF8String]);
        paynuc.set("authTools", [authTools UTF8String]);
        [[SDRequestHelp shareSDRequest] requestWihtFuncName:@"user/resetPhoneNo/v1" errorBlock:^(SDRequestErrorType type) {
            error = YES;
        } successBlock:^{
            [[SDRequestHelp shareSDRequest] dispatchToMainQueue:^{
                [HUD hidden];
                
                NSDictionary *phoneNumDic = [[PayNucHelper sharedInstance] jsonStringToDictionary:userInfo];
                NSString *phoneNum = [phoneNumDic objectForKey:@"phoneNo"];
                
                SuccessPhoneNoViewController *sVC = [[SuccessPhoneNoViewController alloc] initWithNibName:@"SuccessPhoneNoViewController" bundle:nil];
