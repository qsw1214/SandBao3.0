...
#pragma mark - 目前圆饼暂时支持数组数量为2, 超过两组金额数据,暂未判断圆心位置
...
-(void)createCircleLayer{
    ...
    circleRadius = layerFrame.size.width/4 - 2*space;
    tempAngle = 0;
    ...
    for (int i = 0; i<numberArray.count; i++) {
        ...
        //1.现金余额小于消费余额 -> 现金余额圆心偏移
        if ([numberArray[0] floatValue] < [numberArray[1] floatValue]) {
            //调整第一块 饼 的位置
            if (i == 0) {
...
-(void)createCircleInView{
    //文字圆心初始值 固定
    CGPoint centerCircleInit = CGPointMake(layerFrame.size.width/4+space,layerFrame.size.height/2);
    CGFloat centerX = centerCircleInit.x;
    CGFloat centerY = centerCircleInit.y;
...
-(void)createTextShowView{
    ...
    kindLab.textColor = [UIColor lightGrayColor];
    ...
    showOne.textColor = [UIColor lightGrayColor];
    ...
    showTwo.textColor = [UIColor lightGrayColor];
