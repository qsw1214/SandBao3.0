...
{
    //导航渐变条
    navCoverView = [NavCoverView shareNavCoverView:CGRectMake(0, 0, viewSize.width, controllerTop) title:@"银行列表"];
...
-(void)addView:(UIView *)view
{
    ...
    bankTableView = [[UITableView alloc] init];
    bankTableView.delegate = self;
    bankTableView.dataSource = self;
    bankTableView.scrollEnabled = YES;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    BankManageViewController *mBankManageViewController = [[BankManageViewController alloc] init];
    mBankManageViewController.payToolDic = bankArray[indexPath.row];
    [self.navigationController pushViewController:mBankManageViewController animated:YES];
}
