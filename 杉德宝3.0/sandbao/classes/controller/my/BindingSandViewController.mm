...
@interface BindingSandViewController () <UITextFieldDelegate>
{
...
@property (nonatomic, strong) UITextField *sandNumTextField;
...
@property (nonatomic, strong) UITextField *sandCodeTextField;

@end

@implementation BindingSandViewController
...
{
    //导航渐变条
    navCoverView = [NavCoverView shareNavCoverView:CGRectMake(0, 0, viewSize.width, controllerTop) title:@"添加杉德卡"];
...
-(void)addView:(UIView *)view
{
    ...
    titleLabel.text = @"只支持记名卡，已绑定归集的记名卡不可再次绑定";
    titleLabel.textColor = RGBA(255, 102, 57, 1.0);
    ...
    UIView *sandNumView = [[UIView alloc] init];
    sandNumView.backgroundColor = viewColor;
    [inputView addSubview:sandNumView];
    
    UILabel *sandNumLabel = [[UILabel alloc] init];
    sandNumLabel.text = @"卡号";
    sandNumLabel.textColor = titleColor;
    sandNumLabel.font = [UIFont systemFontOfSize:textFieldTextSize];
    [sandNumView addSubview:sandNumLabel];
    
    sandNumTextField = [[UITextField alloc] init];
    sandNumTextField.placeholder = @"输入杉德卡卡号";
    sandNumTextField.text = SHOWTOTEST(@"7280000100000663");
    sandNumTextField.textColor = textFiledColor;
    [sandNumTextField setMinLenght:@"15"];
    [sandNumTextField setMaxLenght:@"19"];
    sandNumTextField.font = [UIFont systemFontOfSize:textFieldTextSize];
    [sandNumView addSubview:sandNumTextField];
    
    [textFieldArray addObject:sandNumTextField];
    ...
    CGFloat titleViewHeight = titleLabelSize.height + 2 * 10;
    
    CGSize sandNumLabelSize = [sandNumLabel.text sizeWithNSStringFont:sandNumLabel.font];
    CGFloat sandNumViewHeight = sandNumLabelSize.height + 2 * upDownSpace;
    CGFloat inputViewHeight = sandNumViewHeight;
    
    CGFloat sandNumTextFieldWidth = commWidth - sandNumLabelSize.width - leftRightSpace;
    CGSize sandNumTextFieldSize = [sandNumTextField sizeThatFits:CGSizeMake(sandNumTextFieldWidth, MAXFLOAT)];
    ...
    [sandNumView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.center.equalTo(inputView);
        make.size.mas_equalTo(CGSizeMake(commWidth, sandNumViewHeight));
    }];
    
    [sandNumLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(sandNumView).offset(0);
        make.centerY.equalTo(sandNumView);
        make.size.mas_equalTo(CGSizeMake(sandNumLabelSize.width, sandNumLabelSize.height));
    }];
    
    [sandNumTextField mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(sandNumLabel.mas_right).offset(leftRightSpace);
        make.centerY.equalTo(sandNumView);
        make.size.mas_equalTo(CGSizeMake(sandNumTextFieldWidth, sandNumTextFieldSize.height));
    }];
    
    [lineView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(sandNumView.mas_bottom).offset(0);
    ...
    [button mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(authToolsView.mas_bottom).offset(30);
...
- (void)setInputResponderChain {
    ...
    sandNumTextField.keyboardType = UIKeyboardTypeNumberPad;
    sandCodeTextField.keyboardType = UIKeyboardTypeNumberPad;
...
//    }
    ...
    if (sandCodeTextField) {
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(textFiledEditChanged:) name:UITextFieldTextDidChangeNotification object:sandCodeTextField];
    }
}
...
- (void)textFiledEditChanged
{
    ...
    if (sandCodeTextField) {
        if ([@"" isEqualToString:sandCodeTextField.text] || sandCodeTextField.text == nil) {
            return [self changeBtnBackGround:nil index:2];
        }
    }
...
-(BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string{
    ...
    if ([textField.placeholder rangeOfString:@"6位校验码"].location !=NSNotFound) {  //杉德卡6位校验码
        return [self restrictionwithTypeStr:OnlyNumberVerifi string:string];
    }
    return YES;
...
- (CGFloat)addAuthToolsView:(UIView *)view
{
    ...
    for (int i = 0; i < authToolsArrayCount; i++) {
        ...
        if ([@"cardCheckCode" isEqualToString:type]) {
            ...
            groupViewHight = [groupView sandCardVerification];
            allHeight = allHeight + groupViewHight;
            sandCodeTextField = groupView.sandCardVerificationTextField;
            [sandCodeTextField setMaxLenght:@"6"];
            sandCodeTextField.text = SHOWTOTEST(@"505236");
            sandCodeTextField.textColor = textFiledColor;
            sandCodeTextField.tag = textFieldTag;
            [textFieldArray addObject:sandCodeTextField];
        }
...
{
    if (sandNumTextField.text.length > [sandNumTextField.maxLenght intValue] ) {
        return [Tool showDialog:@"输入的杉德卡卡号格式不正确，请重新输入。"];
    }
    
    if (sandCodeTextField) {
        if (sandCodeTextField.text.length != [sandCodeTextField.maxLenght intValue] ) {
            return [Tool showDialog:@"输入的校验码格式不正确，请重新输入。"];
        }
    }
    
    
    BOOL payPassRegular = [UITextField payPassWdTooSimple:sandCodeTextField.text];
    if (payPassRegular == NO) {
        [Tool showDialog:@"密码过于简单" defulBlock:^{
            sandCodeTextField.text = @"";
        }];
        return;
    }
...
- (void)bindingSandCard
{
    ...
    [[SDRequestHelp shareSDRequest] dispatchGlobalQuque:^{
        ...
        [authToolDic setValue:sandCodeTextField.text forKey:@"cardCheckCode"];
        ...
        [accountDic setValue:sandNumTextField.text forKey:@"accNo"];
        ...
        [SDLog logTest:[CommParameter sharedInstance].userInfo];
...
- (void)dealloc
{
    ...
    if (sandCodeTextField) {
        [[NSNotificationCenter defaultCenter] removeObserver:self name:UITextFieldTextDidChangeNotification object:sandCodeTextField];
    }
}
