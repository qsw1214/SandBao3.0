...
typedef void(^SPSPayStateBlock)(NSArray *paramArr);
@interface SpsPayViewController ()<SDPayViewDelegate>
{
...
@implementation SpsPayViewController
@synthesize viewSize;
...
- (void)viewDidAppear:(BOOL)animated{
    ...
    if ([CommParameter sharedInstance].realNameFlag == NO) {
        ...
        [Tool showDialog:@"温馨提示" message:@"您未实名认证,暂不能支付" leftBtnString:@"去实名" rightBtnString:@"再想想" leftBlock:^{
            [self pushRealNameVc];
        } rightBlock:^{
...
{
    //导航渐变条
    navCoverView = [NavCoverView shareNavCoverView:CGRectMake(0, 0, viewSize.width, controllerTop) title:@"订单详情(SPS)"];
    [self.view addSubview:navCoverView];
    
    // 2.设置左边的返回item
    UIImage *leftImage = [UIImage imageNamed:@"closeSPS"];
    UIButton *leftBarBtn = [[UIButton alloc] init];
    leftBarBtn.frame = CGRectMake(5, 20 + (40 - leftImage.size.height) / 2, leftImage.size.width, leftImage.size.height);
    leftBarBtn.tag = 101;
    [leftBarBtn setImage:[UIImage imageNamed:@"closeSPS"] forState:UIControlStateNormal];
    [leftBarBtn setImage:[UIImage imageNamed:@"closeSPS"] forState:UIControlStateHighlighted];
...
- (void)pushRealNameVc{
    ...
    mRealNameAuthenticationViewController.isFromSPSrealName = YES;
...
- (void)buttonActionToDoSomething:(id)sender
{
    ...
    switch (btn.tag) {
        case 201:
        {
            ...
            NSNumber *orderMoneyNumber = [orderDic objectForKey:@"amount"];
            NSString *orderMoney = [NSString stringWithFormat:@"¥ %.2f", [orderMoneyNumber floatValue]/100];
            [payView setPayInfo:payModeArray moneyStr:orderMoney orderTypeStr:@"SPS订单"];
            [payView showPayTool];
        }
            break;
        case 202:
...
- (void)payGoBackByisSuccess:(BOOL)success{
    ...
    NSArray *array = [_otherAPPSPSurl componentsSeparatedByString:@"?"];
 ...
 *@return
 */
- (void)pay:(NSString *)param orderPaySuccessBlock:(SPSPayStateBlock)successBlock oederErrorBlock:(SPSPayStateBlock)errorBlock
{
    HUD = [SDMBProgressView showSDMBProgressOnlyLoadingINViewImg:self.view];
    [HUD hidden];
    [SDRequestHelp shareSDRequest].HUD = HUD;
