...
@interface CashWithDrawViewController ()<UITextFieldDelegate>
{
...
@property (nonatomic, strong) UILabel *limitButtomLab;
...
@synthesize button;
@synthesize payView;
@synthesize limitButtomLab;

@synthesize payModeArray;
@synthesize inPayToolDic;
...
-(void)viewWillAppear:(BOOL)animated{
    ...
    //计算转出(待提现)账户额度域
    NSDictionary *limitDic = [cashPayToolDic objectForKey:@"limit"];
    limitFloat = [[PayNucHelper sharedInstance] limitInfo:limitDic]/100;
    limitLabel.text = [NSString stringWithFormat:@"该账户本次最多可提现:%.2f元",limitFloat];
    _userableBalanceFloat = [[[cashPayToolDic objectForKey:@"account"] objectForKey:@"useableBalance"] floatValue]/100;
}
...
{
    //导航渐变条
    navCoverView = [NavCoverView shareNavCoverView:CGRectMake(0, 0, viewSize.width, controllerTop) title:@"提现"];
...
-(void)addView:(UIView *)view
{
    ...
    bankNametLabel.font = [UIFont systemFontOfSize:titleTextSize];
    ...
    feeView.backgroundColor = viewColor;
    [moneyView addSubview:feeView];
    
    UILabel *feeLab = [[UILabel alloc] init];
    feeLab.text = @"提现金额(手续费:)";
    feeLab.textColor = textFiledColor;
    feeLab.font = [UIFont systemFontOfSize:labelTextSize];
    [moneyView addSubview:feeLab];
    ...
    moneyTextField.font = [UIFont systemFontOfSize:textFieldTextSize];
    ...
    //limitButtomvView
    UIView *limitButtomvView = [[UIView alloc] init];
    limitButtomvView.backgroundColor = viewColor;
    [moneyView addSubview:limitButtomvView];
    
    limitButtomLab = [[UILabel alloc] init];
    limitButtomLab.text = @"该卡最低提现金额0.01元";
    limitButtomLab.textColor = textFiledColor;
    limitButtomLab.font = [UIFont systemFontOfSize:labelTextSize];
    [limitButtomvView addSubview:limitButtomLab];
    
    //全部提现
    UILabel *allWithDrawLab = [[UILabel alloc] init];
    allWithDrawLab.text = @"全部提现";
    allWithDrawLab.textColor = RGBA(65, 131, 215, 1.0);
    allWithDrawLab.font = [UIFont systemFontOfSize:labelTextSize];
    [limitButtomvView addSubview:allWithDrawLab];
    
    UIButton *allWithDrawBtn = [[UIButton alloc] init];
    allWithDrawBtn.tag = 3;
    allWithDrawBtn.backgroundColor = [UIColor clearColor];
    [allWithDrawBtn addTarget:self action:@selector(buttonActionToDoSomething:) forControlEvents:UIControlEventTouchUpInside];
    [limitButtomvView addSubview:allWithDrawBtn];
    ...
    CGSize limitButtomViweSize = [limitButtomLab sizeThatFits:CGSizeMake(commWidth, MAXFLOAT)];
    CGSize feeViewsize = [feeLab sizeThatFits:CGSizeMake(commWidth, MAXFLOAT)];
    ...
    CGFloat moneyViewHeight = moneyTextFieldSize.height + 2 * leftRightSpace + feeViewsize.height + 2 * leftRightSpace + limitButtomViweSize.height;
    CGSize allWithDrawLabSize = [allWithDrawLab.text sizeWithNSStringFont:[UIFont systemFontOfSize:labelTextSize]];
    ...
    [feeView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.size.mas_equalTo(CGSizeMake(viewSize.width, feeViewsize.height+2*leftRightSpace));
    }];
    
    [feeLab mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerY.equalTo(feeView.mas_centerY).offset(0);
        make.left.equalTo(feeView).offset(leftRightSpace);
        make.size.mas_equalTo(CGSizeMake(viewSize.width, feeViewsize.height));
    }];
    
    [moneyTitleLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(moneyView).offset(leftRightSpace);
        make.top.equalTo(feeView.mas_bottom).offset(0);
    ...
    [lineView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(moneyTextField.mas_bottom).offset(0);
        make.left.equalTo(moneyView).offset(leftRightSpace);
        make.size.mas_equalTo(CGSizeMake(viewSize.width-leftRightSpace, 1));
    }];
    
    [limitButtomvView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(moneyTextField.mas_bottom).offset(0);
        make.left.equalTo(moneyView).offset(0);
        make.size.mas_equalTo(CGSizeMake(viewSize.width, limitButtomViweSize.height+2*leftRightSpace));
    }];
    
    [limitButtomLab mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerY.equalTo(limitButtomvView.mas_centerY).offset(0);
        make.left.equalTo(limitButtomvView).offset(leftRightSpace);
        make.size.mas_equalTo(CGSizeMake(commWidth, limitButtomViweSize.height));
    }];
    
    
    [allWithDrawLab mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerY.equalTo(limitButtomvView.mas_centerY).offset(0);
        make.right.equalTo(limitButtomvView).offset(-leftRightSpace);
        make.size.mas_equalTo(allWithDrawLabSize);
...
- (void)verificationInput
{
    ...
    if (limitFloat < [moneyTextField.text floatValue]) {
        moneyTextField.text = @"";
        [self changeBtnBackGround:nil];
        limitButtomLab.text = @"金额已超过可提现额度";
        limitButtomLab.textColor = RGBA(255, 102, 57, 1.0);
        return;
    }
    if ([[[cashPayToolDic objectForKey:@"account"]objectForKey:@"useableBalance"] floatValue]/100< [moneyTextField.text floatValue]) {
        moneyTextField.text = @"";
        [self changeBtnBackGround:nil];
        limitButtomLab.text = @"金额已超过该账户可提现余额";
        limitButtomLab.textColor = RGBA(255, 102, 57, 1.0);
        return;
    }
    if ([moneyTextField.text doubleValue]<0.01) {
        moneyTextField.text = @"";
        [self changeBtnBackGround:nil];
        limitButtomLab.text = @"金额不能为零";
        limitButtomLab.textColor = RGBA(255, 102, 57, 1.0);
        return;
    }
    
    
    [payView setPayInfo:(NSArray*)payModeArray moneyStr:[NSString stringWithFormat:@"¥%@",moneyTextField.text] orderTypeStr:@"现金账户提现"];
    [self fee];
...
- (void)buttonActionToDoSomething:(id)sender
{
    ...
    switch (btn.tag) {
        ...
        case 3:
        {
            moneyTextField.text = [NSString stringWithFormat:@"%.2f",limitFloat>_userableBalanceFloat?_userableBalanceFloat:limitFloat];
            limitButtomLab.text = @"该卡最低提现金额0.01元";
            limitButtomLab.textColor = textFiledColor;
...
- (void)payViewSelectPayToolDic:(NSMutableDictionary *)selectPayToolDict{
    inPayToolDic = selectPayToolDict;
    NSString *accNo  = [[inPayToolDic objectForKey:@"account"] objectForKey:@"accNo"];
    NSString *title = [inPayToolDic objectForKey:@"title"];
...
- (void)payViewPwd:(NSString *)pwdStr paySuccessView:(SDPaySuccessAnimationView *)successView{
    ...
    [self recharge:pwdStr paramDic:tempWorkDic rechargeSuccessBlock:^(NSArray *paramArr){
        ...
        dispatch_after(delayTime, dispatch_get_main_queue(), ^{
            ...
            mRechargeResultViewController.viewControllerIndex = CASHBALABNCEDRAW;
        ...
        [[SDRequestHelp shareSDRequest] requestWihtFuncName:@"payTool/getPayTools/v1" errorBlock:^(SDRequestErrorType type) {
            ...
            [[SDRequestHelp shareSDRequest] dispatchToMainQueue:^{
                ...
                if ([payToolsArray count] <= 0) {
                    ...
                    //2.设置VC默认显示的支付
                    inPayToolDic = [[NSMutableDictionary alloc] initWithDictionary:payToolsArrayUsableM[0]];
                    [self resetBankNameLabelAndIconImageView];
                    //设置支付方式列表
...
- (void)fee
{
    ...
    [[SDRequestHelp shareSDRequest] dispatchGlobalQuque:^{
        ...
        NSString *outPayTool = [[PayNucHelper sharedInstance] dictionaryToJson:cashPayToolDic];
        NSString *inPayTool = [[PayNucHelper sharedInstance] dictionaryToJson:inPayToolDic];
...
- (void)recharge:(NSString *)param paramDic:(NSDictionary *)paramDic rechargeSuccessBlock:(CashRechargeStateBlock)successBlock rechagreErrorBlock:(CashRechargeStateBlock)errorBlock
{
    ...
    [[SDRequestHelp shareSDRequest] dispatchGlobalQuque:^{
        ...
        NSMutableDictionary *outPayToolDic = [NSMutableDictionary dictionaryWithDictionary:cashPayToolDic];
