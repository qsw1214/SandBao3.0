//
//  VerifiOldPhoneViewController.m
//  sandbao
//
...
@interface VerifiOldPhoneViewController ()<UITableViewDelegate,UITableViewDataSource>
{
...
@implementation VerifiOldPhoneViewController
...
- (void)settingNavigationBar
{
    ...
    //导航渐变条
    navCoverView = [NavCoverView shareNavCoverView:CGRectMake(0, 0, viewSize.width, controllerTop) title:@"安全验证"];
...
-(void)addView:(UIView *)view
{
    ...
    UILabel *titleLabelBig = [[UILabel alloc] init];
    phoneNoStr = [NSString phoneNumFormat:[CommParameter sharedInstance].phoneNo];
    NSMutableAttributedString *titleText = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:@"您正在为手机%@更换手机号",phoneNoStr]];
    [titleText addAttribute:NSForegroundColorAttributeName value:textFiledColor range:NSMakeRange(0, 6)];
    [titleText addAttribute:NSForegroundColorAttributeName value:[UIColor redColor] range:NSMakeRange(6, 11)];
    [titleText addAttribute:NSForegroundColorAttributeName value:textFiledColor range:NSMakeRange(17, 5)];
    titleLabelBig.attributedText = titleText;
    titleLabelBig.textAlignment = NSTextAlignmentCenter;
    titleLabelBig.font = [UIFont systemFontOfSize:textFieldTextSize];
    [titleView addSubview:titleLabelBig];
    
    UILabel *titleLabel = [[UILabel alloc] init];
    titleLabel.text = @"选择一种方式进行身份校验";
    titleLabel.textColor = [UIColor grayColor];
    titleLabel.textAlignment = NSTextAlignmentCenter;
    titleLabel.font = [UIFont systemFontOfSize:textFieldTextSize-2];
    ...
    CGSize titleLabBigSize = [titleLabelBig sizeThatFits:CGSizeMake(commWidth, MAXFLOAT)];
    CGFloat titleViewHeight = titleLabelSize.height + titleLabBigSize.height + 2 * 15;
...
//    }];
    ...
    [titleLabelBig mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(titleView.mas_top).offset(leftRightSpace);
        make.centerX.equalTo(titleView);
        make.size.mas_equalTo(CGSizeMake(commWidth, titleLabBigSize.height));
    }];
    [titleLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.bottom.equalTo(titleView.mas_bottom).offset(-leftRightSpace);
        make.centerX.equalTo(titleView);
...
{
    VerificationPhoneViewController *pVC = [[VerificationPhoneViewController alloc] init];
    pVC.authGroupDic = authGroupsArray[indexPath.row];
    pVC.phoneNoStrNew = _PhoneNoStrNew;
