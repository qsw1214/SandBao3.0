...
/** 扫描内容的Y值 */
#define scanContent_Y [UIScreen mainScreen].bounds.size.height * 0.24
/** 扫描内容的X值 */
#define scanContent_X [UIScreen mainScreen].bounds.size.width * 0.15

#define layerBounds    [UIScreen mainScreen].bounds

@interface SDScanView()<AVCaptureMetadataOutputObjectsDelegate>{
...
-(void)setLayer:(CALayer*)layer{
    ...
    CGFloat lineimgViewX = ([UIScreen mainScreen].bounds.size.width - lineimg.size.width)/2;
...
//    AVMetadataMachineReadableCodeObject *metadataObj = [metadataObjects objectAtIndex:0];
    ...
    //播放提示音
    [self playSoundEffect:@"SGQRCode.bundle/sound.caf"];
    
    //输出扫码字符串
    if ([_delegate respondsToSelector:@selector(SDScanViewOutputMetadataObjects:)]) {
        [_delegate SDScanViewOutputMetadataObjects:metadataObjects];
    }
