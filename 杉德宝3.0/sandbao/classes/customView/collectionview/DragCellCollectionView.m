...
- (void)handelCellClick:(NSNotification *)notification {
    ...
    ItemCollectionViewCell *cell = notification.object;
    NSIndexPath *indexPath = [self indexPathForCell:cell];
    MinletsData *mMinletsData = [MinletsData shareMinletsData];
    ...
    if (indexPath.section == 0) {
        ...
        [UIView animateWithDuration:0.2 animations:^{
            ...
            [mMinletsData.headArray removeObject:cell.dataModel];
            if (mMinletsData.headArray.count == 0) {
                mMinletsData.isShowHeaderMessage = YES;
            }else {
                mMinletsData.isShowHeaderMessage = NO;
            }
        ...
        NSMutableArray *array = [NSMutableArray arrayWithArray:mMinletsData.dataArray[0]];
        [array addObject:cell.dataModel];
        cell.dataModel.state =  ServeSelected;
        cell.dataModel.isNewAdd = YES;
        mMinletsData.dataArray[0] = array;
        [mMinletsData.headArray addObject:cell.dataModel];
...
- (void)handelCellCancelClick{
    MinletsData *mMinletsData = [MinletsData shareMinletsData];
    
    NSMutableArray *temp = @[].mutableCopy;
    
    //获取数据源
    [temp addObjectsFromArray:mMinletsData.dataArray];
...
- (void)setBeginEditing:(BOOL)beginEditing {
    ...
    [MinletsData shareMinletsData].isEditing = beginEditing;
...
- (void)setCancelEditing:(BOOL)cancelEditing
{
    if (cancelEditing) {
//        [self handelCellCancelClick];
        [[NSNotificationCenter defaultCenter] postNotificationName:notification_CellCancelEditing object:@"QQQQQQQ"];
    }
}
