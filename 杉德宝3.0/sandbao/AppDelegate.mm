...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    NSInteger LoadingResult = [self loadData];
    LoadingResult = 2;
 ...
 *@return LOADING失败/LOADING成功且明登陆引导/LOADING成功且暗登陆引导
 */
- (NSInteger)loadData
{
    //1.load
...
- (void)loadingError:(NSString*)errorStr {
    LoadingErrorMainViewController *mLoadingErrorMainViewController = [[LoadingErrorMainViewController alloc] init];
    mLoadingErrorMainViewController.errorInfo = errorStr;
    self.window.rootViewController = mLoadingErrorMainViewController;
    [self.window makeKeyAndVisible];
}
