...
@implementation SVProgressHUD_sps
...
static SVProgressHUD_sps *sharedView = nil;
...
+ (SVProgressHUD_sps*)sharedView {
	
	if(sharedView == nil)
		sharedView = [[SVProgressHUD_sps alloc] initWithFrame:[[UIScreen mainScreen] bounds]];
...
+ (void)setStatus:(NSString *)string {
	[[SVProgressHUD_sps sharedView] setStatus:string];
}

#pragma mark - Show Methods

+ (void)show {
	[SVProgressHUD_sps showWithStatus:nil networkIndicator:SVProgressHUDShowNetworkIndicator];
}

+ (void)showWithStatus:(NSString *)status {
    [SVProgressHUD_sps showWithStatus:status networkIndicator:SVProgressHUDShowNetworkIndicator];
}

+ (void)showWithMaskType:(SVProgressHUDMaskType)maskType {
    [SVProgressHUD_sps showWithStatus:nil maskType:maskType networkIndicator:SVProgressHUDShowNetworkIndicator];
}

+ (void)showWithStatus:(NSString*)status maskType:(SVProgressHUDMaskType)maskType {
    [SVProgressHUD_sps showWithStatus:status maskType:maskType networkIndicator:SVProgressHUDShowNetworkIndicator];
}

+ (void)showWithStatus:(NSString *)status networkIndicator:(BOOL)show {
    [SVProgressHUD_sps showWithStatus:status maskType:SVProgressHUDMaskTypeNone networkIndicator:show];
}

+ (void)showWithMaskType:(SVProgressHUDMaskType)maskType networkIndicator:(BOOL)show {
    [SVProgressHUD_sps showWithStatus:nil maskType:maskType networkIndicator:show];
}

+ (void)showWithStatus:(NSString*)status maskType:(SVProgressHUDMaskType)maskType networkIndicator:(BOOL)show {
    [[SVProgressHUD_sps sharedView] showWithStatus:status maskType:maskType networkIndicator:show];
}

+ (void)showSuccessWithStatus:(NSString *)string {
    [SVProgressHUD_sps show];
    [SVProgressHUD_sps dismissWithSuccess:string afterDelay:1];
}

+ (void)showErrorWithStatus:(NSString *)string {
    [SVProgressHUD_sps show];
    [SVProgressHUD_sps dismissWithError:string afterDelay:1];
}
...
+ (void)showInView:(UIView*)view {
    [SVProgressHUD_sps showWithStatus:nil maskType:SVProgressHUDMaskTypeNone networkIndicator:SVProgressHUDShowNetworkIndicator];
}

+ (void)showInView:(UIView*)view status:(NSString*)string {
    [SVProgressHUD_sps showWithStatus:string maskType:SVProgressHUDMaskTypeNone networkIndicator:SVProgressHUDShowNetworkIndicator];
}

+ (void)showInView:(UIView*)view status:(NSString*)string networkIndicator:(BOOL)show {
    [SVProgressHUD_sps showWithStatus:string maskType:SVProgressHUDMaskTypeNone networkIndicator:show];
}
...
+ (void)showInView:(UIView*)view status:(NSString*)string networkIndicator:(BOOL)show posY:(CGFloat)posY maskType:(SVProgressHUDMaskType)hudMaskType {
    [SVProgressHUD_sps showWithStatus:string maskType:hudMaskType networkIndicator:show];
}

#pragma mark - Dismiss Methods

+ (void)dismiss {
	[[SVProgressHUD_sps sharedView] dismiss];
}

+ (void)dismissWithSuccess:(NSString*)successString {
	[[SVProgressHUD_sps sharedView] dismissWithStatus:successString error:NO];
}

+ (void)dismissWithSuccess:(NSString *)successString afterDelay:(NSTimeInterval)seconds {
    [[SVProgressHUD_sps sharedView] dismissWithStatus:successString error:NO afterDelay:seconds];
}

+ (void)dismissWithError:(NSString*)errorString {
	[[SVProgressHUD_sps sharedView] dismissWithStatus:errorString error:YES];
}

+ (void)dismissWithError:(NSString *)errorString afterDelay:(NSTimeInterval)seconds {
    [[SVProgressHUD_sps sharedView] dismissWithStatus:errorString error:YES afterDelay:seconds];
}
...
- (id)initWithFrame:(CGRect)frame {
    ...
    if ((self = [super initWithFrame:frame])) {
        ...
        self.backgroundColor = [UIColor colorWithRed:210/255 green:210/255 blue:210/255 alpha:0.5];
		self.alpha = 1;
...
- (void)showWithStatus:(NSString*)string maskType:(SVProgressHUDMaskType)hudMaskType networkIndicator:(BOOL)show {
    ...
    if(![self isKeyWindow]) {
        ...
        [[UIApplication sharedApplication].windows enumerateObjectsWithOptions:NSEnumerationReverse usingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
            ...
            if(window.windowLevel == UIWindowLevelNormal && ![[window class] isEqual:[SVProgressHUD_sps class]]) {
...
- (void)dismissWithStatus:(NSString*)string error:(BOOL)error {
	[self dismissWithStatus:string error:error afterDelay:1.5];
}
...
- (CGFloat)visibleKeyboardHeight {
    ...
    for (UIWindow *testWindow in [[UIApplication sharedApplication] windows]) {
        if(![[testWindow class] isEqual:[UIWindow class]] && ![[testWindow class] isEqual:[SVProgressHUD_sps class]]) {
