...
- (void)addButtonsToView: (UIView *)container
{
    ...
    for (int i=0; i<[buttonTitles count]; i++) {
        ...
        [closeButton setBackgroundImage:[self imageWithColor:[UIColor colorWithRed:90.0/255.0 green:140.0/255.0 blue:1.0 alpha:1] size:closeButton.frame.size] forState:UIControlStateHighlighted];
...
#pragma mark passWordViewConfig
- (void)passWordViewConfig
{
    UIView *demoView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 280, 120)];
    
    UILabel *messageL = [[UILabel alloc] initWithFrame:CGRectMake(0, 10, CGRectGetWidth(demoView.frame), 30)];
    [messageL setTextAlignment:NSTextAlignmentCenter];
    [messageL setText:@"请输入支付密码"];
    
    UIView *lineV = [[UIView alloc] initWithFrame:CGRectMake(10, CGRectGetHeight(messageL.frame)+10, CGRectGetWidth(demoView.frame) - 20 , 1)];
    lineV.backgroundColor = [UIColor colorWithRed:190.0/255.0 green:190.0/255.0  blue:190.0/255.0  alpha:1];
    
    tf = [[UITextField alloc] initWithFrame:CGRectMake(0, 0, 0, 0)];
    [tf setKeyboardType:UIKeyboardTypeNumberPad];
    tf.delegate = self;
    for (NSInteger i = 0; i<6; i++) {
        UIView *passCube = [[UIView alloc] initWithFrame:CGRectMake(20 + i*40, 60, 38, 38)];
        passCube.tag = i+2;
        passCube.backgroundColor = [UIColor whiteColor];
        [demoView addSubview:passCube];
    }
    
    
    [demoView addSubview:tf];
    [demoView addSubview:lineV];
    [demoView addSubview:messageL];
            ...
            for (UIView *view in self.subviews) {
                for (UIView *v1 in view.subviews) {
                    for (UIView *v2 in v1.subviews) {
                        ...
                        if (v2.tag == textField.text.length +1) {
        ...
        for (UIView *view in self.subviews) {
            for (UIView *v1 in view.subviews) {
                for (UIView *v2 in v1.subviews) {
                    ...
                    if (v2.tag - 2 == range.location && v2.subviews.count == 0) {
                        //  NSLog(@"tag    %d",v2.tag);
                        UIView *bv = [[UIView alloc] initWithFrame:CGRectMake(CGRectGetWidth(v2.frame)/2 -9, CGRectGetHeight(v2.frame)/2 - 9, 18, 18)];
                        [bv setBackgroundColor:[UIColor blackColor]];
