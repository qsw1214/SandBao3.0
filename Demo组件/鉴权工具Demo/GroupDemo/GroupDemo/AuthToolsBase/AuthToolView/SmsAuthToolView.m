...
- (CGFloat)shortMsgAndPhoneNumVerification
{
    ...
    if (_bottomShow) {
        ...
        bottomSpaceView.backgroundColor = [UIColor colorWithRed:(242/255.0) green:(242/255.0) blue:(242/255.0) alpha:1.0];
    ...
    shortMsgVerificationBtn.selected = NO;
    [shortMsgVerificationBtn setTitleColor:[UIColor colorWithRed:(65/255.0) green:(131/255.0) blue:(215/255.0) alpha:1.0] forState:UIControlStateNormal];
    [shortMsgVerificationBtn setTitleColor:[UIColor colorWithRed:(200/255.0) green:(199/255.0) blue:(204/255.0) alpha:1.0] forState:UIControlStateSelected];
    [shortMsgVerificationBtn addTarget:self action:@selector(changeBtnState:) forControlEvents:UIControlEventTouchUpInside];
...
- (void)refreshTime
{
    ...
    if (_timeCount < 0) {
        [timer invalidate];
        timer = nil;
        [shortMsgVerificationBtn setUserInteractionEnabled:YES];
        shortMsgVerificationBtn.selected = NO;
        [shortMsgVerificationBtn setTitle:@"重发一次" forState: UIControlStateNormal];
        
    } else {
        NSString *time = [NSString stringWithFormat:@"%.f秒", _timeCount];
        [shortMsgVerificationBtn setUserInteractionEnabled:NO];
        [shortMsgVerificationBtn setTitle:time forState: UIControlStateNormal];
    }
}
